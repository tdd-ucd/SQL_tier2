...
FROM Facilities
WHERE membercost != 0.0 and membercost <= monthlymaintenance*1/5
...
SELECT f.name,SUM(CASE WHEN m.firstname = 'GUEST' THEN f.guestcost ELSE f.membercost END) AS revenue
FROM bookings b
...
SELECT f.name,COUNT(b.memid) AS total_booking
FROM bookings b
LEFT JOIN facilities f
USING (facid)
WHERE b.memid != 0
...
SELECT f.name, STRFTIME('%m', starttime) AS month, COUNT(bookid) total_bookings
FROM bookings b
LEFT JOIN facilities f
USING (facid)
WHERE b.memid != 0
